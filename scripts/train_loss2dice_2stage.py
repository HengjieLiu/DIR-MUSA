"""
Train registration models with Dice loss for 2-stage registration
(Stage 1: coarse registration at r2 resolution, Stage 2: fine registration at r1 resolution)
Note: 
    1. This script assume the stage1 model is pre-trained at r2 resolution using train_loss2dice_1stage.py, 
        and we freeze the stage1 model weights and train the stage2 model at r1 resolution using this script.
    2. To provide a fair comparison with loss3musa, for loss2dice, we only enforce Dice loss on the bone structures (seg_b)
        The purpose is to show that using Dice loss on bone structures CANNOT resolve the plausibility issue!!!
    ...
    Args:
        ...
        --model-load-stage1: Model path for weight loading (stage1, pre-trained at r2)
    ...
    Example:
        python train_loss2dice_2stage.py \
            ...
            --model-load-stage1 /path/to/stage1_model.pth \
...
parser = argparse.ArgumentParser(description='Train DIR-MUSA registration models, Dice loss for 2-stage registration (coarse r2 + fine r1)')
...
parser.add_argument('--seg-path-b', required=True, help='Path to bone segmentations (for training Dice loss and validation)')
parser.add_argument('--model-type', required=True, choices=['01voxelmorph-vf', '02resunet-vf', '03lkunet-vf-lk09', '04transmorph-vf', '05dualprnet-vf'], help='Model type to train, 06lapirn-vf needs its own training script')
parser.add_argument('--model-load-stage1', required=True, help='Model path for weight loading (stage1, pre-trained at r2 resolution)')
parser.add_argument('--model-load-stage2', required=True, help="Model path for weight loading (stage2), or 'from-scratch' for random initialization")
...
parser.add_argument('--gamma', required=True, type=float, dest='weight_gamma', help='Weight of Dice loss')
...
# Setups for data
trn_files = musa.utils_io.read_file_list(args.trn_list)
val_files = musa.utils_io.read_file_list(args.val_list)

# For Dice loss, note we use seg_b to compare with musa's regularization
trn_dataset = musa.utils_dataloader.myDataset_trn(trn_files, args.vol_path, path_seg=args.seg_path_b)
...
# Set up spatial transformer and composer for image warping and DVF composition
spatial_transformer_r1 = musa.warp_modules.SpatialTransformer(input_shape_r1)
spatial_transformer_r1.to(device)
composer_r1 = musa.warp_modules.ComposeDVF(input_shape_r1)
...
model_stage1.eval()  # Stage1 is frozen (pre-trained at r2 resolution)
...
# Add Dice loss
func_losses.append(musa.utils_loss.DiceLoss().loss)

weights = [1, args.weight_lambda, args.weight_gamma]
print(f"Loss weights: similarity=1, deformation={args.weight_lambda}, dice={args.weight_gamma}")


###### Initialize training (with support for continue training from checkpoint)

# Load pre-trained stage1 model (coarse registration at r2)
...
if args.checkpoint_path is None: # Initialize
    ...
    if args.model_load_stage2 == 'from-scratch':
        print(f"Initializing stage2 model from scratch")
    else:
    ...
    list_epoch_loss_dice = checkpoint.get('list_epoch_loss_dice', [])
...
for epoch in range(epoch_start, args.epochs):
    ...
    for trn_step, data in enumerate(trn_generator):
        ...
        ### unpack data
        data = [d.to(device) for d in data] # (moving, fixed, moving_seg_b, fixed_seg_b)
        (moving, fixed, moving_seg_b, fixed_seg_b) = data
        ...
        with torch.no_grad():
            ...
            moving_r2 = musa.warp_modules.vol_downsamplex2(moving_r2)
            fixed_r2 = musa.warp_modules.vol_downsamplex2(fixed_r2)
            ...
            dvf_r1_stage1 = musa.warp_modules.dvf_upsample(dvf_r2)
        ...
        # Convert segmentation to one-hot encoding (assuming 9 classes for bone segmentation)
        moving_seg_b_oh = F.one_hot(moving_seg_b.long(), num_classes=9).float().permute(0, 4, 1, 2, 3)
        fixed_seg_b_oh  = F.one_hot(fixed_seg_b.long(), num_classes=9).float().permute(0, 4, 1, 2, 3)
        # Use 'bilinear' for training while 'nearest' for evaluation (see voxelmorph and transmorph repo)
        """
        Hering et al. https://doi.org/10.1016/j.media.2021.102139
        During training, we use linear interpolation to warp the one-hot segmentation masks since this results in a smoother loss function at the border of the segmentation. With nearest neighbor interpolation, the loss of each voxel can either be one or zero. Linear interpolation allows for probabilistic loss values between zero and one.
        """
        deformed_seg_b_oh = spatial_transformer_r1(moving_seg_b_oh, dvf_composed, mode='bilinear')

        ### loss calculation
        # Loss is calculated on the final composed result
        loss_sim        = func_losses[0](fixed, deformed_final) * weights[0]
        loss_dvf_smooth = func_losses[1](dvf_composed) * weights[1]
        loss_dice       = func_losses[2](fixed_seg_b_oh, deformed_seg_b_oh) * weights[2]
        loss = loss_sim + loss_dvf_smooth + loss_dice
    ...
    epoch_loss_dice /= args.steps_per_epoch
    ...
    list_epoch_loss_dice.append(epoch_loss_dice)
    
    # Organize trn_info
    epoch_info = 'Epoch %d/%d' % (epoch+1, args.epochs)
    time_info = '%.4f sec/step, %.4f sec/epoch' % (epoch_step_time, epoch_step_time*args.steps_per_epoch)
    loss_info = 'loss: %.4e, loss_sim: %.4e, loss_dvf_smooth: %.4e, loss_dice: %.4e' % \
                (epoch_loss, epoch_loss_sim, epoch_loss_dvf_smooth, epoch_loss_dice)
    ...
    if (epoch+1) % args.epoch_save == 0:
        ...
        checkpoint = {
            ...
            'list_epoch_loss_dice': list_epoch_loss_dice,
