"""
Train registration models with Dice loss for 1-stage registration (work both on full/half resolution)
Note: 
    1. To provide a fair comparison with loss3musa, for loss2dice, we only enforce Dice loss on the bone structures (seg_b)
    ...
    Example:
        python train_loss2dice_1stage.py \
...
parser = argparse.ArgumentParser(description='Train DIR-MUSA registration models, Dice loss for 1-stage registration (work both on full/half resolution)')
...
spatial_transformer_r1 = musa.warp_modules.SpatialTransformer((160, 160, 192))
...
for epoch in range(epoch_start, args.epochs):
    ...
    for trn_step, data in enumerate(trn_generator):
        ...
        for i in range(down_scale_cnt):
            moving = musa.warp_modules.vol_downsamplex2(moving)
            fixed  = musa.warp_modules.vol_downsamplex2(fixed)
        ...
        if down_scale_cnt > 0:
            dvf_r1 = musa.warp_modules.dvf_upsample(dvf)
        else:
            dvf_r1 = dvf
        
        ### use dvf (r1) to warp the (r1) segmentations
        ...
        """
        deformed_seg_b_oh = spatial_transformer_r1(moving_seg_b_oh, dvf_r1, mode='bilinear')
    ...
    if (epoch+1) % args.epoch_val == 0:
        ...
        with torch.no_grad():
            ...
            for val_step, data in enumerate(val_generator):
                ...
                for i in range(down_scale_cnt):
                    dvf = musa.warp_modules.dvf_upsample(dvf)
